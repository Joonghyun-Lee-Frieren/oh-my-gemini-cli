...
const MODEL_PRICING_PER_1M: Record<string, ModelPricingPer1M> = {
  ...
  'gemini-3.1-flash': { input: 0.35, output: 0.7 },
...
export function getModelPricing(model?: string): ModelPricingPer1M {
  ...
  if (normalized.includes('flash-lite')) return MODEL_PRICING_PER_1M['gemini-3.1-flash-lite'];
  if (normalized.includes('flash')) return MODEL_PRICING_PER_1M['gemini-3.1-flash'];
  if (normalized.includes('pro')) return MODEL_PRICING_PER_1M['gemini-3.1-pro'];
