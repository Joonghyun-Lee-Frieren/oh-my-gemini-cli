...
## 하네스 정렬 업데이트 (2026-03-03)

`oh-my-codex` 운영 흐름 중 Gemini 확장 네이티브 구조에서 구현 가능한 제어를 OmG에 반영했습니다.

- `/omg:reasoning` + `$reasoning`: 추론 강도(`low`, `medium`, `high`, `xhigh`) 전환
- `/omg:doctor` + `$doctor`: 설치/상태/팀 준비도 진단 (`team` 범위 포함)
- `/omg:approval` + `$approval`: 승인 포스처(`suggest`, `auto`, `full-auto`) 전환
- `/omg:cancel`: 안전 중지 + 재개 핸드오프 별칭 명령

경계 참고:
- OmG는 확장 상태/프롬프트 레이어에서 정책을 저장하고 적용합니다.
- 호스트 CLI의 런타임 플래그(예: 승인/추론 엔진 설정)는 Gemini CLI 런타임 설정이 별도로 필요합니다.
...
| `/omg:reasoning` | 추론 강도 조회/전환 (`low/medium/high/xhigh`) | 깊은 분석 전 또는 지연/비용 튜닝이 필요할 때 |
...
| `$reasoning` | 추론 강도 프로파일 전환 | 깊이/비용 포스처 + 다음 명령 |
| `$approval` | 승인 포스처 전환 | 가드레일 요약 + 위험 정책 |
| `$doctor` | 설치/준비도 진단 | 진단 보드 + 우선 복구 순서 |
